    ...
    public partial class DefineColors2Page : Page
    {
        ...
        public void updateFrames(object sender, EventArgs e)
        {
            try
            {
                ...
                IMG_ShowMinColor.Source = PreviewColor(HSV_PiecesColors.Player2_MCvS_Data[(int)ColorValArray.Minimum]);
                IMG_ShowMaxColor.Source = PreviewColor(HSV_PiecesColors.Player2_MCvS_Data[(int)ColorValArray.Maximum]);
            }
            catch (Exception ex)
            {
                ComponentDispatcher.ThreadIdle -= new EventHandler(updateFrames);
