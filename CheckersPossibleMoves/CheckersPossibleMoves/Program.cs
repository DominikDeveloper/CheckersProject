...
namespace CheckersPossibleMoves
{
    class Program
    {
        ...
        private static void CheckRightJumpForWhite(int[,] board, int row, int col) //checks valid right-hand jump
        {
        ...
        private static void CheckLeftJumpForWhite(int[,] board, int row, int col) //check valid left-hand jump
        {
            if (board[row + 1, col - 1] >= (int)FieldValues.BlackMen &&
                board[row + 1, col - 1] == (int)FieldValues.BlackMen &&
                board[row + 2, col - 2] < 2)
